...
const PHRASES=[
  ...
  {id:9,emoji:"🏥",cat:"navigation",text:{en:"Where is the hospital?",ar:"أين المستشفى؟",es:"¿Dónde está el hospital?",zh:"医院在哪？",hi:"अस्पताल कहाँ?",fr:"Où est l'hôpital ?"},gesture:{en:"Draw cross on arm, look around.",ar:"ارسم + على ذراعك.",es:"Cruz en brazo.",zh:"臂上画十字。",hi:"बांह पर क्रॉस।",fr:"Croix sur bras."}},
  {id:10,emoji:"🗺️",cat:"navigation",text:{en:"A map, please",ar:"خريطة من فضلك",es:"Un mapa",zh:"请给地图",hi:"नक्शा दीजिए",fr:"Une carte SVP"},gesture:{en:"Hands flat, palms up, like a map.",ar:"يداك مسطحتان كخريطة.",es:"Manos planas.",zh:"双手平放。",hi:"हाथ सपाट।",fr:"Mains à plat."}},
  ...
  {id:13,emoji:"💬",cat:"general",text:{en:"How can I help?",ar:"كيف أساعدك؟",es:"¿Cómo ayudo?",zh:"需要帮忙吗？",hi:"कैसे मदद करूं?",fr:"Comment aider ?"},gesture:{en:"Point to self, lifting motion.",ar:"أشر لنفسك ثم ارفع.",es:"Señálate, levanta.",zh:"指自己，托起。",hi:"अपनी ओर इशारा।",fr:"Pointez, levez."}},
      ...
      <div style={{flex:1,overflowY:"auto",paddingBottom:"calc(80px + env(safe-area-inset-bottom, 0px))",overflowX:"hidden",WebkitOverflowScrolling:"touch"}}>
        ...
        {tab==="settings"&&<div className="fi" style={{padding:20}}>
          ...
          <div><div style={{fontSize:11,fontWeight:700,color:c.tx2,textTransform:"uppercase",letterSpacing:1,marginBottom:8}}>{t.accessLabel}</div>
            {[{k:"wheelchair",i:"♿",l:t.wheelchair,d:t.wheelchairDesc||"Avoids stairs, prefers wider corridors. Routes use lifts only.",co:"#059669",bg:dark?"rgba(5,150,105,.12)":"#ECFDF5"},{k:"vision",i:"👁",l:t.vision,d:t.visionDesc||"Larger text, higher contrast, thicker map lines.",co:"#D97706",bg:dark?"rgba(217,119,6,.12)":"#FFFBEB"},{k:"hearing",i:"👂",l:t.hearing,d:t.hearingDesc||"Visual-only navigation. Flashing direction cues replace audio. Auto-opens sign language phrases for quick communication with staff.",co:"#7C3AED",bg:dark?"rgba(124,58,237,.12)":"#F5F3FF"},{k:"cognitive",i:"🧠",l:t.cognitive,d:t.cognitiveDesc||"One simple instruction per step. No distances or extra info.",co:"#EA580C",bg:dark?"rgba(234,88,12,.12)":"#FFF7ED"}].map(m=><button key={m.k} onClick={()=>toggleAcc(m.k)} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 14px",background:acc[m.k]?m.bg:c.w,border:`1.5px solid ${acc[m.k]?m.co:c.bd}`,borderRadius:12,cursor:"pointer",width:"100%",marginBottom:6,textAlign:"left"}}><span style={{fontSize:22}}>{m.i}</span><div style={{flex:1}}><div style={{fontWeight:700,fontSize:base-1,color:acc[m.k]?m.co:c.tx}}>{m.l}</div><div style={{fontSize:11,color:c.tx2,lineHeight:1.4}}>{m.d}</div></div><div style={{width:38,height:22,borderRadius:11,background:acc[m.k]?m.co:"#D1D5DB",position:"relative",flexShrink:0}}><div style={{width:16,height:16,borderRadius:"50%",background:dark?"#0B1120":"#fff",position:"absolute",top:3,left:acc[m.k]?19:3,transition:"all .2s",boxShadow:"0 1px 2px rgba(0,0,0,.2)"}}/></div></button>)}
          </div>
        </div>}
      </div>
