...
export default function MedNav(){
  ...
  const [dark,setDark]=useState(false);const [listening,setListening]=useState(false);const [voiceMsg,setVoiceMsg]=useState("");
  ...
  const navToDept=(roomId)=>{setEndRoom(roomId);setMapFloor(ROOMS[roomId].floor);setTab("navigate")};const startVoice=()=>{const SR=window.SpeechRecognition||window.webkitSpeechRecognition;if(!SR){setVoiceMsg("Voice not supported on this browser");setTimeout(()=>setVoiceMsg(""),3000);return}const rec=new SR();rec.lang={en:"en-US",ar:"ar-SA",es:"es-ES",zh:"zh-CN",hi:"hi-IN",fr:"fr-FR"}[lang]||"en-US";rec.continuous=false;rec.interimResults=false;setListening(true);setVoiceMsg("Listening...");rec.onresult=(e)=>{const said=e.results[0][0].transcript.toLowerCase().trim();const match=Object.entries(ROOMS).find(([id,r])=>{const label=(rl[r.labelKey]||id).toLowerCase();return label.includes(said)||said.includes(label)||id.toLowerCase().includes(said)});if(match){setEndRoom(match[0]);setMapFloor(ROOMS[match[0]].floor);setVoiceMsg("Found: "+(rl[match[1].labelKey]||match[0]));setTimeout(()=>setVoiceMsg(""),2500)}else{setVoiceMsg("Couldnt find: "+said);setTimeout(()=>setVoiceMsg(""),3000)}setListening(false)};rec.onerror=()=>{setVoiceMsg("Voice error - try again");setTimeout(()=>setVoiceMsg(""),3000);setListening(false)};rec.onend=()=>setListening(false);rec.start()};
          ...
          <div style={{padding:"12px 16px 0"}}>
            {!route?<div style={{display:"flex",flexDirection:"column",gap:10}}>
              <div style={{display:"flex",gap:10}}>
                ...
                <div style={{flex:1,position:"relative"}}><label style={{fontSize:11,fontWeight:600,color:c.tx2,marginBottom:4,display:"flex",alignItems:"center",justifyContent:"space-between"}}>{t.to}<button onClick={startVoice} disabled={listening} style={{background:listening?"#7C3AED":c.ac,border:"none",borderRadius:"50%",width:24,height:24,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:11,padding:0,animation:listening?"pulse 1s infinite":"none"}}>🎤</button></label>{voiceMsg&&<div style={{position:"absolute",top:-30,right:0,background:c.tx,color:c.w,padding:"4px 10px",borderRadius:8,fontSize:11,whiteSpace:"nowrap",zIndex:10}}>{voiceMsg}</div>}<select value={endRoom} onChange={e=>{setEndRoom(e.target.value);if(e.target.value)setMapFloor(ROOMS[e.target.value].floor)}} style={{width:"100%",padding:"10px 30px 10px 12px",borderRadius:10,border:`1.5px solid ${c.bd}`,background:c.w,color:c.tx,fontSize:base-1,outline:"none"}}><option value="">— {t.selectDest} —</option>{Object.entries(ROOMS).filter(([id])=>id!==startRoom).map(([id,r])=><option key={id} value={id}>{ICONS[r.type]} {rl[r.labelKey]||id} ({r.floor===0?"G":"F"+r.floor})</option>)}</select></div>
              </div>
